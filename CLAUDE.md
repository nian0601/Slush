...
Claude's own in-application verification is limited to: launching the built exe, optionally sending the `2` key (switches to `AssetEditorLayout`) or relying on `SkipStartScreen` to jump straight into gameplay, and taking a screenshot to confirm. Simulating mouse movement/clicks to drive the UI (selecting list rows, dragging, clicking buttons) is out of scope for Claude to do itself — it's slow, fragile (coordinates drift across window moves/resizes, DPI, stale docking layouts), and easy to misread. For any verification step that needs more interaction than a keypress, describe what to check and hand it to the user to click through manually rather than automating it.
