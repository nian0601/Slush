...
template <typename T>
FW_Rect<T> MakeRectFromTopLeft(const FW_Vector2<T>& aTopLeft, const FW_Vector2<T>& someExtents)
{
	FW_Vector2<T> halfExtents = someExtents / static_cast<T>(2.f);
	return MakeRectFromCenter(aTopLeft + halfExtents, someExtents);
}

template <typename T>
FW_Rect<T> MakeRectFromCenter(const FW_Vector2<T>& aCenter, const FW_Vector2<T>& someExtents)
{
	FW_Rect<T> rect;
